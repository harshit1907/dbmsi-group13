...
public class GraphDB implements GlobalConst {
  ...
  public int getNodeCnt() throws HFException, HFBufMgrException, HFDiskMgrException, IOException, InvalidSlotNumberException, InvalidTupleSizeException {
	  NodeHeapFile n = new NodeHeapFile(name+"n");
	  return n.getNodeCnt();
  }
  ...
  public int getDestinationCnt() throws IOException, InvalidSlotNumberException {
	  ...
	  EID eid = epage.firstEdge();
	  HashSet<NID> destSet = new HashSet<NID>();
	  while(eid!=null) {
		  Edge aedge = epage.getEdge(eid);
		  destSet.add(aedge.getDestination());
		  eid = epage.nextEdge(eid);
	  }
	  return destSet.size();
  }
  public int getSourceCnt() throws IOException, InvalidSlotNumberException {
	  /*EdgeHeapFile e = new EdgeHeapFile(name);
	  EScan escan = new EScan(e); */
	  EHFPage epage = new EHFPage();
	  //PageId fpage = e._firstDirPageId;
