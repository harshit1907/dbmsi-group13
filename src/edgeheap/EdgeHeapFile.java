...
public class EdgeHeapFile implements Filetype,  GlobalConst {
	 ...
	 */
	private boolean  _findDataPage( EID rid,
					...
					Exception
	{
		...
		{// Start While01
					...
					currentDataPageRid = currentDirPage.nextEdge(currentDataPageRid))
			{
				...
				{
					atuple = currentDataPage.returnEdge(rid);
	...
	public int getEdgeCnt() 
		...
		while(currentDirPageId.pid != INVALID_PAGE)
		{
			...
			EID rid = new EID();
			Edge atuple;
			for (rid = currentDirPage.firstEdge();
					rid != null;	// rid==NULL means no more record
					rid = currentDirPage.nextEdge(rid))
			{
				atuple = currentDirPage.getEdge(rid);
				DataPageInfo dpinfo = new DataPageInfo(atuple);
			...
			IOException, KeyTooLongException, KeyNotMatchException, LeafInsertRecException, IndexInsertRecException, ConstructPageException, UnpinPageException, PinPageException, NodeNotMatchException, ConvertException, DeleteRecException, IndexSearchException, IteratorException, LeafDeleteException, InsertException, GetFileEntryException, AddFileEntryException, PageUnpinnedException, InvalidFrameNumberException, HashEntryNotFoundException, ReplacerException, InvalidTypeException
	{
		...
		EID rid;
		rid = currentDataPage.insertEdge(recPtr);
	 ...
	 */
	public boolean deleteEdge(EID rid)  
			throws InvalidSlotNumberException, 
			InvalidTupleSizeException, 
			HFException, 
			HFBufMgrException,
	...
	/*	
		if(SystemDefs.JavabaseDB.btEdgeWeight!=null) { 
            SystemDefs.JavabaseDB.btEdgeWeight = new BTreeFile(SystemDefs.JavabaseDBName+"_BTreeEdgeWeightIndex", AttrType.attrInteger, 8, 1delete); 
            System.out.println("I am here"+SystemDefs.JavabaseDB.ehfile.getEdge(rid).getWeight());
	...
	*/	boolean status;
		...
		currentDataPage.deleteEdge(rid);
	 ...
	 */
	public boolean updateEdge(EID rid, Edge newtuple) 
			throws InvalidSlotNumberException, 
			...
			Exception
	{
		...
		if(status != true) return status;	// record not found
		Edge atuple = new Edge();
		atuple = dataPage.returnEdge(rid);
	...
	public EID getEID(Edge nd, String sLabel, String dLabel) throws InvalidSlotNumberException, InvalidTupleSizeException, HFException, HFDiskMgrException, HFBufMgrException, Exception {
		boolean OK = true;
		boolean FAIL = false;
		EScan scan = null;
		boolean status = OK;
		...
		if (status == OK) {
			...
			while (!done) {
				...
				if (!done && status == OK) {
					...
					{
						NID src = node.getSource();
						NID dest = node.getDestination();
						if(src!=null && dest!=null) {
							String source = SystemDefs.JavabaseDB.nhfile.getNode(src).getLabel();
							String destination = SystemDefs.JavabaseDB.nhfile.getNode(dest).getLabel();
							if(source.equalsIgnoreCase(sLabel)&&destination.equalsIgnoreCase(dLabel)) 
							{
								done = true;
								found=1;
							}
