...
//             for( Pair<EdgeQueryPojo,EdgeQueryPojo> tmpLi : childQuery)
//                {
...
//                }
            ...
            while(edgeQuery.size()>=1)
            {
                ...
                List<Pair<EdgeQueryPojo,EdgeQueryPojo>>  children_nested = new ArrayList<Pair<EdgeQueryPojo,EdgeQueryPojo>>();
                ...
                while (iterQueryEdge.hasNext()) {
                    ...
                    while ((t_inside = nlj_inside.get_next()) != null) {
                       ...
                       // System.out.println("label is "+label);
                        ...
                        //System.out.println("destination data is"+destNid.pageNo);
                        String dest_label1=SystemDefs.JavabaseDB.nhfile.getNode(destNid).getLabel();
                        tmp.setDestLabel(dest_label1);
                      //  System.out.println("destination label is"+dest_label1);
                        tmp.setEdgelabel(label);
                        tmp.setUniqueKey(t_inside.getIntFld(9));
                        int wt = t_inside.getIntFld(1);
                        //System.out.println(edgeQuery.get(0).getKey()+"is key");
                        if (edgeQuery.get(0).getKey() == 1) {
                            if (edgeQuery.get(0).getEdgelabel().equalsIgnoreCase(label)) {
                                
                                children_nested.add(new Pair<EdgeQueryPojo,EdgeQueryPojo>(nextIterQuery.getLeft(),tmp));
                                if (edgeQuery.size() == 1) {
                                    EdgeQueryPojo destNidPojo = new EdgeQueryPojo();
                                    destNidPojo.setNd(destNid);
                                    ansNids.add(new Pair<EdgeQueryPojo,EdgeQueryPojo>(nextIterQuery.getLeft(),tmp));
                                }
                            }
             ...
             //   System.out.println(children_nested.size()+"SADAS");
                edgeQuery.remove(0);
...
//                }
//                System.out.println("printint");
//                 for( Pair<EdgeQueryPojo,EdgeQueryPojo> tmpLi : children_nested)
//                    {
//                        System.out.println("(Head: "+tmpLi.getLeft().getDestLabel()+" | Tail: "+tmpLi.getRight().getDestLabel()+"),");
//                    }
            }
...
//            // CREATE Temp Heap File
            ...
            for (int j=0;j<ansNids.size();j++) {
                nhp.setStrFld(1, String.format("%05d", Integer.parseInt(ansNids.get(j).getLeft().getDestLabel()))
                        + "_" + String.format("%05d", Integer.parseInt(ansNids.get(j).getRight().getDestLabel())));
            ...
            if (Type.equalsIgnoreCase("b")) {
                ...
                while (t != null) {                   
                    try {
                        EdgeQueryPojo head = new EdgeQueryPojo();
                        EdgeQueryPojo tail = new EdgeQueryPojo();
                        head.setDestLabel(String.valueOf(Integer.parseInt(t.getStrFld(1).substring(0,5))));
                        tail.setDestLabel(String.valueOf(Integer.parseInt(t.getStrFld(1).substring(6,11))));
                        ansNidsSorted.add(new Pair<EdgeQueryPojo, EdgeQueryPojo>(head, tail));
                        t = sort.get_next();
                    }
                    catch (Exception e) {
                        e.printStackTrace();
