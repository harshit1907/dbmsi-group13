 ...
 */
public class ZBTreeFile extends ZIndexFile
    ...
    private ZBTreeHeaderPage headerPage;
     ...
     * of this btree file.
     */
    public ZBTreeHeaderPage getHeaderPage() {
        return headerPage;
    }
     ...
     */
    public ZBTreeFile(String filename)
            throws GetFileEntryException,
            PinPageException,
        ...
        headerPage = new ZBTreeHeaderPage(headerPageId);
        dbname = filename;
      /*
       *
    ...
    public ZBTreeFile(String filename, int keytype,
        ...
        {
            headerPage = new ZBTreeHeaderPage();
            ...
            headerPage.setType(NodeType.ZBTHEAD);
        } else {
            headerPage = new ZBTreeHeaderPage(headerPageId);
        }
    ...
    private void _destroyFile(PageId pageno)
        ...
        ZBTSortedPage sortedPage;
        Page page = pinPage(pageno);
        sortedPage = new ZBTSortedPage(page, headerPage.get_keyType());

        if (sortedPage.getType() == NodeType.INDEX) {
            ZBTIndexPage indexPage = new ZBTIndexPage(page, headerPage.get_keyType());
            NID nid = new NID();
            PageId childId;
    ...
    private void updateHeader(PageId newRoot)
        ...
        ZBTreeHeaderPage header;
        PageId old_data;


        header = new ZBTreeHeaderPage(pinPage(headerPageId));
    ...
    public void insert(KeyClass key, NID nid) throws KeyTooLongException, KeyNotMatchException,
            ...
            IOException, DescriptorNotMatchException,
            LeafInsertDescException, IndexInsertDescException, DeleteDescException {
        DescriptorDataEntry newRootEntry;

        if (ZBT.getKeyLength(key) > headerPage.get_maxKeySize()) {
        ...
        if (headerPage.get_rootId().pid == INVALID_PAGE) {
            ...
            ZBTLeafPage newRootPage;
            NID dummynid;

            newRootPage = new ZBTLeafPage(headerPage.get_keyType());
        ...
        if (newRootEntry != null) {
            ...
            newRootPage = new ZBTIndexPage(headerPage.get_keyType());
            newRootPageId = newRootPage.getCurPage();
    ...
    private DescriptorDataEntry _insert(KeyClass key, NID nid, PageId currentPageId) throws PinPageException,
            ...
            IndexInsertRecException, KeyNotMatchException, NodeNotMatchException, InsertException, DescriptorNotMatchException, IndexInsertDescException, DeleteDescException, LeafInsertDescException {
        ZBTSortedPage currentPage;
        Page page;
        DescriptorDataEntry upEntry;

        page = pinPage(currentPageId);
        currentPage = new ZBTSortedPage(page, headerPage.get_keyType());
        ...
        if (currentPage.getType() == NodeType.INDEX) {
            ZBTIndexPage currentIndexPage = new ZBTIndexPage(page, headerPage.get_keyType());
            ...
            currentIndexPage = new ZBTIndexPage(pinPage(currentPageId), headerPage.get_keyType());
            ...
            if (currentIndexPage.available_space() >=
                    ZBT.getKeyDataLength(upEntry.key, NodeType.INDEX)) {
            ...
            DescriptorDataEntry undoEntry = null;
            ...
            if (ZBT.keyCompare(upEntry.key, tmpEntry.key) >= 0) {
                ...
                tmpEntry = ZBT.getEntryFromBytes(currentIndexPage.getpage(),
            ...
            ZBTLeafPage currentLeafPage =
                    new ZBTLeafPage(page, headerPage.get_keyType());
            ...
            // check whether there can still be entries inserted on that page
            if (currentLeafPage.available_space() >= ZBT.getKeyDataLength(key, NodeType.LEAF)) {
                // no split has occurred
            ...
            if (rightPageId.pid != INVALID_PAGE) {
                ZBTLeafPage rightPage;
                rightPage = new ZBTLeafPage(rightPageId, headerPage.get_keyType());
            ...
            if (ZBT.keyCompare(key, undoEntry.key) < 0) {
                //undo the final record
            ...
            // fill upEntry
            tmpEntry = newLeafPage.getFirst(firstNid);
            upEntry = new DescriptorDataEntry(tmpEntry.key, newLeafPageId);
    ...
    public boolean Delete(KeyClass key, NID nid) throws DeleteFashionException, LeafRedistributeException,
            ...
            DeleteRecException, IndexSearchException, IOException, DeleteDescException,
            DescriptorNotMatchException, InsertDescException, IndexInsertDescException {
    ...
    public ZBTLeafPage findRunStart (KeyClass lo_key, NID startnid) throws IOException, IteratorException,
            KeyNotMatchException, ConstructPageException, PinPageException, UnpinPageException, DescriptorNotMatchException {
        ZBTLeafPage pageLeaf;
        ZBTIndexPage pageIndex;
        Page page;
        ZBTSortedPage sortPage;
        PageId pageno;
        PageId curpageno=null;                // iterator
        PageId prevpageno;
        PageId nextpageno;
        RID curRid;
        DescriptorDataEntry curEntry;
        ...
        page= pinPage(pageno);
        sortPage = new ZBTSortedPage(page, headerPage.get_keyType());
        ...
        while (sortPage.getType() == NodeType.INDEX) {
            pageIndex = new ZBTIndexPage(page, headerPage.get_keyType());
            prevpageno = pageIndex.getPrevPage();
            curEntry= pageIndex.getFirst(startnid);
            while ( curEntry!=null && lo_key != null && ZBT.keyCompare(curEntry.key, lo_key) < 0) {
        ...
        pageLeaf = new ZBTLeafPage(page, headerPage.get_keyType() );
        ...
        while (curEntry==null) {
            ...
            pageno = nextpageno;
            pageLeaf=  new ZBTLeafPage( pinPage(pageno), headerPage.get_keyType());
            curEntry=pageLeaf.getFirst(startnid);
        }
        ...
        while (ZBT.keyCompare(curEntry.key, lo_key) < 0) {
    ...
    private boolean NaiveDelete( KeyClass key, NID nid) throws LeafDeleteException,
            ...
            DescriptorNotMatchException, ConstructPageException {
        ...
        while (true) {
            while (entry == null) { // have to go right
                ...
                leafPage = new ZBTLeafPage(pinPage(nextpage),
            ...
            if (leafPage.delEntry(new DescriptorDataEntry(key, nid))) {
            ...
            leafPage = new ZBTLeafPage(pinPage(nextpage), headerPage.get_keyType());
    ...
    private boolean FullDelete (KeyClass key,  NID nid) throws IndexInsertRecException,
            ...
            UnpinPageException, IteratorException, InsertDescException, DeleteDescException,
            IndexInsertDescException, DescriptorNotMatchException {
        try {
            if ( trace !=null) {
                trace.writeBytes( "DELETE " + nid.pageNo + " " + nid.slotNo
                        + " " +  key +lineSep);
    ...
    private KeyClass _Delete ( KeyClass key, NID nid, PageId currentPageId, PageId parentPageId)
            ...
            IteratorException,
            PinPageException,
            IOException, InsertDescException, DeleteDescException, IndexInsertDescException, DescriptorNotMatchException {
        ...
        if (sortPage.getType()==NodeType.LEAF ) {
            ...
            PageId nextpage;
            ZBTLeafPage leafPage;
            leafPage= new ZBTLeafPage(page, headerPage.get_keyType());
            ...
            while((tmpEntry!=null) && (ZBT.keyCompare(key,tmpEntry.key)>=0)) {
                ...
                if ( leafPage.delEntry(new DescriptorDataEntry(key, nid)) ) {
                    ...
                    if ( (4+leafPage.available_space()) <= ((MAX_SPACE-HFPage.DPFIXED)/2) ) {
                        ...
                        // get a sibling
                        ZBTIndexPage  parentPage;
                        parentPage = new ZBTIndexPage(pinPage(parentPageId), headerPage.get_keyType());
                        PageId siblingPageId=new PageId();
                        ZBTLeafPage siblingPage;
                        int direction;
                        ...
                        siblingPage = new ZBTLeafPage(pinPage(siblingPageId), headerPage.get_keyType());
                        ...
                        if (siblingPage.redistribute( leafPage, parentPage, direction, deletedKey)) {
                            ...
                            DescriptorDataEntry oldChildEntry;
                            if (direction==-1) {
                            ...
                            ZBTLeafPage leftChild;
                            ZBTLeafPage rightChild;
                            if (direction==-1) {
                            ...
                            if ( rightChild.getNextPage().pid != INVALID_PAGE) {
                                ZBTLeafPage nextLeafPage = new ZBTLeafPage(
	 ...
	 */
        ...
        if (  sortPage.getType() == NodeType.INDEX ) {
            ...
            indexPage=new ZBTIndexPage(pinPage(currentPageId), headerPage.get_keyType());
            ...
            if (indexPage.getCurPage().pid == headerPage.get_rootId().pid) {
                ...
                if (indexPage.numberOfRecords() == 0) {
                    ZBTSortedPage childPage;
                    childPage = new ZBTSortedPage(indexPage.getPrevPage(),
                            headerPage.get_keyType());
                    if ( trace !=null ) {
            ...
            if ((4 /*sizeof slot*/ +indexPage.available_space()) <=
                ...
                // get a sibling
                ZBTIndexPage  parentPage;
                parentPage = new ZBTIndexPage(pinPage(parentPageId),
                        headerPage.get_keyType());
                PageId siblingPageId=new PageId();
                ZBTIndexPage siblingPage;
                int direction;
                ...
                siblingPage = new ZBTIndexPage( pinPage(siblingPageId), headerPage.get_keyType());
                int pushKeySize=0;
                if (direction==1) {
                    pushKeySize = ZBT.getKeyLength
                ...
                if (siblingPage.redistribute(indexPage,parentPage,
                    ...
                    // merge the two children
                    ZBTIndexPage leftChild, rightChild;
                    if (direction==-1) {
                        leftChild = siblingPage;
                    ...
                    // move all entries from rightChild to leftChild
                    for (DescriptorDataEntry tmpEntry=rightChild.getFirst(firstNid);
                         tmpEntry != null;
                         tmpEntry=rightChild.getFirst(firstNid) ) {
    ...
    public ZBTFileScan new_scan(KeyClass lo_key, KeyClass hi_key)
            throws IOException,
            KeyNotMatchException,
            IteratorException,
            ConstructPageException,
            PinPageException,
            UnpinPageException, DescriptorNotMatchException {
        ZBTFileScan scan = new ZBTFileScan();
        if ( headerPage.get_rootId().pid==INVALID_PAGE) {
            scan.leafPage=null;
            return scan;
        }
    ...
    public void trace_children(PageId id)
        ...
        if( trace!=null ) {
            ...
            KeyClass key;
            DescriptorDataEntry entry;
            sortedPage = new ZBTSortedPage( pinPage( id), headerPage.get_keyType());

            // Now print all the child nodes of the page.
            if( sortedPage.getType()==NodeType.INDEX) {
                ZBTIndexPage indexPage = new ZBTIndexPage(sortedPage,headerPage.get_keyType());
                ...
                ZBTLeafPage leafPage = new ZBTLeafPage(sortedPage,headerPage.get_keyType());
